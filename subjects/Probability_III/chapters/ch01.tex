...
\begin{theorem}
    Given any probability measure $\mu$ on $(\R^{n},\cB(\R^{n}))$, there exists a measurable map $T:([0,1],\cB([0,1]),\lambda) \to (\R^{n},\cB(\R^{n}))$ such that $\mu = \lambda \circ T^{-1}$. That is, the pushforward measure of $\lambda$ under $T$ is $\mu$. 
\end{theorem}