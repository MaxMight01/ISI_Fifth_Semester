...
Given a smooth map $F:N \to M$ between smooth manifolds, we can define the map $F_{\ast,p}:T_{p}N \to T_{F(p)}M$ as $D \mapsto F_{\ast,p}(D)$, where $F_{\ast,p}(D)(f) = D(f \circ F)$ for all $f \in C_{F(p)}^{\infty}$. This is the pushforward of $f$ at $p$. Moreover, if $G:M \to P$ is another smooth map, then $(G \circ F)_{\ast,p} = G_{\ast,F(p)} \circ F_{\ast,p}$.
